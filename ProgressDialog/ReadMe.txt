================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : ProgressDialog プロジェクトの概要
===============================================================================

この ProgressDialog アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、ProgressDialog アプリケーションを構成する各ファイルの内容の概要が含まれています。

ProgressDialog.vcxproj
...
ProgressDialog.vcxproj.filters
...
ProgressDialog.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CProgressDialogApp アプリケーション クラスを宣言します。

ProgressDialog.cpp
    これはメインのアプリケーション ソース ファイルで、CProgressDialogApp アプリケーション クラスが含まれています。
...
res\ProgressDialog.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル ProgressDialog.rc にインクルードされます。

res\ProgressDialog.rc2
...
ProgressDialogDlg.h, ProgressDialogDlg.cpp - ダイアログ
    これらのファイルには、CProgressDialogDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる ProgressDialog.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、ProgressDialog.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

ProgressDialog.manifest
